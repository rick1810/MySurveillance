...
public class MJPEG extends Thread {
	...
	public MJPEG(MySurveillance ms, Camera cam, String address, String path, String username, String password) {
	...
	private void record() {
		...
		String name = cam.getName() + "/" + (now.getYear() + "-" + sMon + "-" + sDay + " " + sHour + sMin) + ".mjpeg";
		ms.fileManager.addData(ms.savePath + "/" + name, stream);
	...
	public void run() {
		...
		try {
			...
			byte[] s = new byte[0];
			s = ByteManager.append(s, "GET " + path + " HTTP/1.1\n");
